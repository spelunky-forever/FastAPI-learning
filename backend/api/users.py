...
def get_users_infor(page_params: CommonPaginationParams, db = Depends(get_db)):
    return UserCRUD.get_users_infor(keyword=page_params.keyword, last=page_params.last, limit=page_params.limit, db=db)

@router.get("/{user_id}", response_model= UserSchema.UserBase)
def get_user_by_id(user_id: int, qry: str = None, db = Depends(get_db)):
    result = UserCRUD.get_user_by_id(user_id, db)
    if result is None:
...
def create_users(new_user: UserSchema.UserCreate, db = Depends(get_db)):
    result = UserCRUD.create_users(new_user, db)
    if result is None:
...
def delete_users(current_user:UserModel = Depends(check_user_id), db = Depends(get_db)):
    result = UserCRUD.delete_users(current_user, db)
    return result
...
def update_user_optional(user_update: UserSchema.UserBase, db = Depends(get_db), current_user:UserModel = Depends(check_user_id)):
    result = UserCRUD.update_user_optional(user_update, db, current_user)
    return result
...
def change_password(update_info: UserSchema.UserPasswordUpdate, db = Depends(get_db), current_user:UserModel = Depends(check_user_id)):
    result = UserCRUD.change_password(update_info, db, current_user)
    if result is None:
